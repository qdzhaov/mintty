...
do_csi(uchar c)
{
  ...
  switch (CPAIR(cterm->esc_mod, c)) {
    ...
    when CPAIR('"', 'q'):  /* DECSCA: select character protection attribute */
      switch (arg0) {
        when 0 or 2: cterm->curs.attr.attr &= ~ATTR_PROTECTED;
        when 1: cterm->curs.attr.attr |= ATTR_PROTECTED;
      }
